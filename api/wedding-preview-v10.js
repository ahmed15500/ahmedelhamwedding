...
function buildPng() {
  ...
  text(raw,stride,'AHMED & ELHAM',250,9,C.purple);
  ...
  text(raw,stride,'6:00 PM',455,6,C.muted);
...
module.exports = (req,res)=>{ const img=buildPng(); res.setHeader('Content-Type','image/png'); res.setHeader('Content-Length',String(img.length)); res.setHeader('Cache-Control','public, max-age=0, must-revalidate'); res.statusCode=200; res.end(img); };
